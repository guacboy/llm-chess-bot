...
def train(
    ...
    # Switches on training-specific behaviour.
    model.train()

    # Reads the loss and decides how much to nudge each weight.
    ...
    for epoch in range(epochs):
        ...
        # Wraps the range and draws a live progress bar in the terminal.
        batch_range = tqdm(
        ...
        for i in batch_range:
            ...
            # win  = negative * negative = positive loss -> model is penalized for low prob
            ...
            # Backpropagation: Works out how each weight contributed
...
def load_model(model: ChessNet, device: torch.device) -> ChessNet:
    ...
    If no saved weights exist, the model keeps its random initialization.
    """
